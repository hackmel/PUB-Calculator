...
<div class="form">
	<div id="tab" class="tab">
		<ul>
			<li><a href="#tabs-1"> Search Tenant </a></li>
				...
				<table class="tbl" id="tenants">
					...
					<tbody>
						<c:forEach varStatus="status" items="${tenantList}" var="item">
							...
							<td><a href="displayTenants.do?id=${item.id}">${item.lastName}
							</a></td>

							<td><a href="displayTenants.do?id=${item.id}">${item.firstName}
							</a></td>

							<td><a href="displayTenants.do?id=${item.id}">${item.middleName}
							</a></td>
							</tr>
						</c:forEach>
					...
					<table>
						<tr>
							<c:if test="${firstPage==false}">
								<th><a
									href="${pageContext.request.contextPath}/paginateTenantList.do?page=previous"
									name="previous" class="button">Previous</a></th>
							</c:if>
							<c:if test="${lastPage==false}">
								<th><a
									href="${pageContext.request.contextPath}/paginateTenantList.do?page=next"
